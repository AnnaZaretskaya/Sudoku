    ...
    var _loop_1 = function (i) {
        var _loop_2 = function (j) {
            if ((!(userChoice[i][j] == null)) && (game[i][j] == null) && (userChoice[i][j] != baseGame[i][j])) {
                ...
                setTimeout(function () {
                    table.rows[i].cells[j].innerHTML = baseGame[i][j];
                }, 1000);
                setTimeout(function () {
                    table.rows[i].cells[j].style.background = 'rgba(235, 235, 235, 1)';
                }, 2000);
...
$(".table").click(function (event) {
    ...
    //coordsX коррдината Х появления меню, перерасчет недопускает выпадания  за границы при маленьких экранах
    var coordsX = function () {
        if (($(window).width() < 670) && (event.pageX > ($(window).width()) - 85)) {
            console.log(event.pageX - 90);
    ...
    if (game[curRow][curCol]) {
        ...
        $(".popup").fadeIn(100);
        $('.popup').css({ 'top': event.pageY, 'left': coordsX });
    }
});
